...
@app.callback(
    [
        Output('dropdown-contas-receitas', 'options'),
        Output('dropdown-contas-receitas', 'value'),
    ],
    Input('store-receitas', 'data')
)
def conta_dropdown(dict_receitas):
    df = pd.DataFrame(dict_receitas)
    conta = df.Conta.unique().tolist()
    conta_unique = [{'label': i, 'value': i} for i in conta]
    return (conta_unique, conta)
...
@app.callback(
    ...
    [
        Input('store-receitas', 'data'),
        Input('dropdown-contas-receitas', 'value'),
    ]
)
def tabela_receitas(dict_receitas, contas):
    df = pd.DataFrame(dict_receitas)
